...
```mermaid
flowchart LR
    A["用户浏览器"] --> B["Vercel 静态站点"]
    B --> C["React 前端应用"]
    C --> D["本地工具计算模块"]
    C --> E["浏览器 Web API"]
    B --> F["Vercel CDN 分发"]
    B -. "后续可扩展" .-> G["Vercel Serverless Functions"]
    D --> H["编解码与格式化逻辑"]
    D --> I["Hash 与 UUID 生成逻辑"]
    E --> J["Clipboard API"]
    E --> K["Intl 与 Date API"]
    E --> L["网络信息展示占位能力"]
```
...
- 部署平台：Vercel，使用静态前端部署模式，依托其 CDN 提供快速分发
- 可扩展性：未来如果需要真实 IP、DNS、HTTP 检测等远程工具，可增补 Vercel Serverless Functions
...
- 构建命令：`npm run build`
- 输出目录：`dist`
- 部署类型：Vite 单页应用静态部署到 Vercel
- 路由策略：使用 `vercel.json` 将所有非静态资源请求重写到 `/index.html`，保证 React Router 刷新不报 404
- 缓存策略：静态构建产物使用 Vercel 默认静态缓存，HTML 入口由平台自动管理
- 环境变量：若后续接入服务端能力，统一通过 Vercel 项目环境变量管理，不在前端硬编码敏感信息

建议的 `vercel.json` 规则如下：
...
- 保持纯前端部署兼容性，可直接部署到 Vercel 等静态托管平台
- 优先选择体积可控的依赖，避免影响 Vercel 冷启动与静态资源加载速度
...
- 远程能力优先落在 Vercel Serverless Functions，保持部署链路统一
